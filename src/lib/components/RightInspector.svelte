...
{#if instance?.rightPanelOpen}
	<aside class="sticky top-[57px] hidden h-[calc(100vh-57px)] w-72 shrink-0 border-l border-[var(--border-color)] bg-[var(--background-color)] xl:flex xl:flex-col">
		<div class="flex items-center gap-1 border-b border-[var(--border-color)] p-2">
			{#each [
				...
				<button
					...
					class={"rounded-md px-2 py-1 text-[10px] font-medium " + (instance.rightPanelMode === mode.id ? "bg-[var(--hover-background)]" : "")}
				>{mode.label}</button>
			{/each}
			<button type="button" onclick={() => workspaceUi.toggleRightPanel()} class="ml-auto rounded-md px-2 py-1 text-xs hover:bg-[var(--hover-background)]">×</button>
		</div>
		...
		<div class="flex-1 overflow-y-auto p-3">
			{#if instance.rightPanelMode === "context"}
				...
				{:else}
					<p class="mt-3 text-xs leading-5 text-[var(--text-muted)]">No project is pinned to this tab. Select a project or boot a project workspace preset.</p>
				{/if}
				...
				<div class="mt-4">
					...
					<p class="mt-1 text-[11px] text-[var(--text-muted)]">{instance.tabs.length} tabs · {instance.bookmarks.length} bookmarks</p>
				</div>
			{:else if instance.rightPanelMode === "bookmarks"}
				...
				<div class="mt-3 space-y-2">
					{#each instance.bookmarks as bookmark}
						<button type="button" onclick={() => openHref(bookmark.href)} class="block w-full rounded-md border border-[var(--border-color)] px-3 py-2 text-left text-xs hover:bg-[var(--hover-background)]">{bookmark.title}</button>
					{:else}
			...
			{:else if instance.rightPanelMode === "queries"}
				...
				<div class="mt-3 space-y-2">
					{#each relevantQueries as query}
						<button type="button" onclick={() => goto("/queries?query=" + query.id + (selectedProject ? "&project=" + selectedProject.id : ""))} class="block w-full rounded-md border border-[var(--border-color)] px-3 py-2 text-left">
			...
			{:else}
				...
				<div class="mt-3 space-y-3">
					...
					<button type="button" onclick={importUiState} disabled={!importText} class="w-full rounded-md border border-[var(--border-color)] px-3 py-2 text-xs hover:bg-[var(--hover-background)] disabled:opacity-50">Import UI state</button>
