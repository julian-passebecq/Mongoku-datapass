...
<div class="border-b border-[var(--border-color)] bg-[var(--background-color)] px-3 py-1.5">
	<div class="flex items-center gap-1 overflow-x-auto">
		<a href={base + (foilMode ? "/foil" : "/")} class="mr-1 shrink-0 text-[10px] font-semibold uppercase tracking-[0.15em] text-[var(--text-muted)] no-underline">
			{foilMode ? "FOIL" : "Tools"}
		</a>
		{#each tools as tool}
		...
		<div class="ml-auto flex shrink-0 gap-1">
			{#if foilMode}
				<a href={base + "/"} class="rounded-md px-2 py-1 text-[11px] no-underline hover:bg-[var(--hover-background)]">Global</a>
			{/if}
