	...
	<div class="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
		<div>
			...
			<p class="mt-2 max-w-3xl text-sm text-[var(--text-muted)]">
				Versioned project instruction profiles that can be associated with AI-role nodes. Profiles are part of the
				canonical workspace JSON and persist in Mongo when control writes are enabled.
			</p>
		</div>
				...
				<div class="flex items-start justify-between gap-4">
					<div>
						<p class="text-xs text-[var(--text-muted)]">
							{projects.find((project) => project.id === profile.projectId)?.name}
						</p>
