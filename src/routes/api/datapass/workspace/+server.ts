...
import { commitDirectWorkspace, getWorkspaceIdentity } from "$lib/server/datapassHistory";
		...
		return json(
			{
				ok: false,
				error:
					"Datapass control writes are disabled. Set DATAPASS_CONTROL_WRITE_ENABLED=true on your local/private instance.",
			},
			{ status: 403 },
		);
	}
