...
<section class="space-y-6">
	<div>
		...
		<p class="mt-2 max-w-4xl text-sm leading-6 text-[var(--text-muted)]">
			Accepted AI changes and direct JSON commits append immutable snapshots. Restoring an older snapshot creates a new
			revision instead of deleting later history.
		</p>
	</div>
		...
		<div class="grid gap-5 xl:grid-cols-2">
			<section class="rounded-xl border border-[var(--border-color)]">
				<div class="border-b border-[var(--border-color)] px-4 py-3">
					<h2 class="text-sm font-semibold">A · revision {data.comparison.left.revision}</h2>
				</div>
				<pre class="max-h-[540px] overflow-auto p-4 text-[10px] leading-5">{JSON.stringify(
						data.comparison.left.workspace,
						null,
						2,
					)}</pre>
			</section>
			<section class="rounded-xl border border-[var(--border-color)]">
				<div class="border-b border-[var(--border-color)] px-4 py-3">
					<h2 class="text-sm font-semibold">B · revision {data.comparison.right.revision}</h2>
				</div>
				<pre class="max-h-[540px] overflow-auto p-4 text-[10px] leading-5">{JSON.stringify(
						data.comparison.right.workspace,
						null,
						2,
					)}</pre>
			</section>
				...
				{#each data.revisions as revision, __eachIndex3 (__eachIndex3)}
					<div class="flex items-start justify-between gap-4 px-4 py-3">
						...
						<button
							...
							disabled={busy || !data.controlWritesEnabled || revision.revision === data.identity.revision}
							class="shrink-0 rounded-md border border-[var(--border-color)] px-2 py-1 text-[10px] disabled:opacity-40"
							>Restore as new</button
						>
					</div>
				{:else}
					<p class="p-4 text-xs text-[var(--text-muted)]">
						No canonical revisions yet. Initialize the control database or accept a reviewed ChangeSet first.
					</p>
				{/each}
			</div>
		</section>
				...
				{#each data.activity as event, __eachIndex4 (__eachIndex4)}
					<div class="px-4 py-3">
						...
						<p class="mt-1 text-[10px] text-[var(--text-muted)]">
							r{event.revision} · {event.source} · {event.createdAt}
						</p>
					</div>
				{:else}
