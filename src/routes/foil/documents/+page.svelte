...
<section class="space-y-6">
	<div>
		...
		<p class="mt-2 max-w-4xl text-sm leading-6 text-[var(--text-muted)]">
			Metadata/provenance browser only. Large PDF/PPTX/PNG/XLSX/ZIP assets belong in future provider-neutral object
			storage, not MongoDB.
		</p>
	</div>
			...
			<article class="rounded-xl border border-[var(--border-color)] p-5">
				<div class="flex items-start justify-between gap-4">
					<div>
						<p class="text-[10px] font-semibold uppercase tracking-wide text-[var(--text-muted)]">
							{text(row, "artifactType") || "artifact"}
						</p>
				...
				{#if text(row, "repository")}
					<p class="mt-3 text-[10px] text-[var(--text-muted)]">
						{text(row, "repository")} · {text(row, "branch")} · {text(row, "commit")}
					</p>
				{/if}
