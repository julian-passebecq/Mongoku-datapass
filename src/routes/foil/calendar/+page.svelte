...
<section class="space-y-6">
	<div>
		...
		<p class="mt-2 max-w-4xl text-sm leading-6 text-[var(--text-muted)]">
			Supports one-off, recurring and condition-review scheduling contracts. This page never invokes External Audit or Self-Audit automatically.
		</p>
	</div>

	<div class="grid gap-3 md:grid-cols-3">
		<div class="rounded-xl border border-[var(--border-color)] p-4"><p class="text-[10px] uppercase text-[var(--text-muted)]">ONE_OFF</p><p class="mt-2 text-xs">Explicit due/review date.</p></div>
		<div class="rounded-xl border border-[var(--border-color)] p-4"><p class="text-[10px] uppercase text-[var(--text-muted)]">RECURRING</p><p class="mt-2 text-xs">RRULE / cadence with next due state.</p></div>
		<div class="rounded-xl border border-[var(--border-color)] p-4"><p class="text-[10px] uppercase text-[var(--text-muted)]">CONDITION_REVIEW</p><p class="mt-2 text-xs">Review trigger only; no automatic audit execution.</p></div>
	</div>

	{#each report.sections.filter((section) => !section.trace.resolved) as section}
		<div class="rounded-xl border border-dashed border-[var(--border-color)] p-4 text-xs text-[var(--text-muted)]">{section.label}: {section.trace.message}</div>
	{/each}
	...
	<div class="grid gap-4 xl:grid-cols-2">
						...
						<div class="px-4 py-3">
							<div class="flex items-start justify-between gap-3">
								<div>
									...
									<p class="mt-1 text-[11px] text-[var(--text-muted)]">{text(row, "nextAction") || text(row, "currentStep")}</p>
								</div>
		...
		{:else}
			<div class="rounded-xl border border-dashed border-[var(--border-color)] p-6 text-sm text-[var(--text-muted)]">No scheduled FOIL items returned.</div>
		{/each}
	</div>
</section>
