<script lang="ts">
	...
	const propagationUnassessed = $derived(
		propagation?.sections.find((section) => section.id === "unassessed")?.rows ?? [],
	);
	...
	function impactTargets(row: Record<string, unknown>): string[] {
		...
		return value.map((target) => {
			...
			if (target && typeof target === "object") {
				...
				return (
					String(record.name ?? record.target ?? record.id ?? "target") +
					" · " +
					String(record.status ?? record.state ?? "DIRTY")
				);
			}
...
<section class="space-y-6">
	<div>
		...
		<p class="mt-2 max-w-4xl text-sm leading-6 text-[var(--text-muted)]">
			Project hierarchy, authority routing, physical resources, software/runtime and propagation are deliberately
			separate views. They are related, but they are not equivalent graphs.
		</p>
	</div>
				...
				{#each projectRows as row}
					<article class="rounded-xl border border-[var(--border-color)] p-4">
						...
						{#if list(row, "relatedProjects").length > 0}
							<p class="mt-3 text-[10px] text-[var(--text-muted)]">
								Related: {list(row, "relatedProjects").join(", ")}
							</p>
						{/if}
					</article>
				{:else}
					<div
						class="rounded-xl border border-dashed border-[var(--border-color)] p-6 text-sm text-[var(--text-muted)]"
					>
						Project hierarchy source unavailable.
					</div>
				{/each}
			</div>
		</div>
		...
		<div class="space-y-4">
			<div class="rounded-xl border border-[var(--border-color)] p-5">
				<div class="flex flex-wrap items-center justify-center gap-3 text-xs">
					<span class="rounded-lg border border-[var(--border-color)] px-4 py-3 font-semibold"
					...
					<div class="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
						<div>
							...
							<p class="mt-2 text-xs leading-5 text-[var(--text-muted)]">
								{text(row, "purpose") || text(row, "routingRule") || text(row, "authorityRule")}
							</p>
						</div>
						<span class="rounded-full border border-[var(--border-color)] px-2 py-0.5 text-[10px]"
							>{text(row, "status")}</span
						>
					</div>
				...
				<section class="rounded-xl border border-[var(--border-color)] p-5">
					<div>
						...
						<h2 class="mt-1 text-base font-semibold">
							{text(project, "recommendedDisplayName") || text(project, "name")}
						</h2>
						<p class="mt-1 text-[10px] text-[var(--text-muted)]">
							{text(project, "_id")} · {text(project, "externalId")}
						</p>
					</div>
					...
					{#each repositories as repo}
						<div class="rounded-lg bg-[var(--hover-background)] p-3">
							...
							<p class="mt-1 text-[9px] text-[var(--text-muted)]">
								{text(repo, "defaultBranch")} · {text(repo, "currentHead") || text(repo, "verifiedHeadCommit")}
							</p>
						</div>
					{:else}
					...
					{#each projectRows.filter((row) => row.deployment || row.runtime || row.productionDeployment || row.currentPreview) as row}
						<div class="rounded-lg bg-[var(--hover-background)] p-3">
							...
							<pre class="mt-2 max-h-40 overflow-auto text-[9px] leading-4">{JSON.stringify(
									row.deployment ?? row.runtime ?? row.currentPreview ?? row.productionDeployment,
									null,
									2,
								)}</pre>
						</div>
	...
	{:else}
		<div class="space-y-4">
			<div class="rounded-xl border border-[var(--border-color)] p-5 text-xs text-[var(--text-muted)]">
				New source → primary authority → impact analysis → dependent targets DIRTY → reviewed propagation →
				verification. No source is synchronized merely because this view is opened.
				{#if propagationUnassessed.length > 0}
					<p class="mt-2 font-medium">
						{propagationUnassessed.length} PM event(s) currently have no propagation assessment metadata; this is coverage
						debt, not NO_IMPACT.
					</p>
				{/if}
			</div>
