...
<section class="space-y-6">
	<div class="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
		<div>
			...
			<h1 class="mt-2 text-3xl font-semibold tracking-tight">{project ? project.name + " calendar" : "Workspace calendar"}</h1>
			<p class="mt-2 text-sm text-[var(--text-muted)]">Due-date view generated from the saved Mongo query <code>{data.queryId}</code>.</p>
		</div>
		<div class="flex flex-wrap gap-2">
			...
			<span class="rounded-full border border-[var(--border-color)] px-2.5 py-1 text-xs text-[var(--text-muted)]">{data.items.length} items</span>
		</div>
	</div>
	...
	<div class="grid gap-4 xl:grid-cols-2">
					...
					{#each items as item}
						<div class="px-4 py-3">
							...
							<div class="mt-2 flex flex-wrap gap-1">
								{#each item.tags as tag}<span class="rounded border border-[var(--border-color)] px-1.5 py-0.5 text-[10px]">#{tag}</span>{/each}
							</div>
						</div>
					{/each}
				</div>
		...
		{:else}
			<div class="rounded-xl border border-dashed border-[var(--border-color)] p-8 text-sm text-[var(--text-muted)]">No dated open work for this scope.</div>
		{/each}
	</div>
</section>
