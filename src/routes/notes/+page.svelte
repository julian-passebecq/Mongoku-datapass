	...
	<div class="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
		<div>
			...
			<h1 class="mt-2 text-3xl font-semibold tracking-tight">
				{project ? project.name + " notes" : "Workspace notes"}
			</h1>
			<p class="mt-2 text-sm text-[var(--text-muted)]">
				Notes, decisions and research surfaced by <code>{data.queryId}</code>.
			</p>
		</div>
	...
	<div class="grid gap-4 xl:grid-cols-2">
		...
		{:else}
			<div class="rounded-xl border border-dashed border-[var(--border-color)] p-8 text-sm text-[var(--text-muted)]">
				No notes, decisions or research items for this scope.
			</div>
		{/each}
	</div>
</section>
